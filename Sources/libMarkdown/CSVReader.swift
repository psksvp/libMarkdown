...
public class CSVReader
{
  ...
  public subscript(row: Int, col: Int) -> String
  {
    ...
    guard col < rows[row].count else {return ""}
    return self.rows[row][col].trimmingCharacters(in: .whitespacesAndNewlines)
  }
  
  public func column(_ c: Int) -> [String]
  {
    (0 ..< rows.count).map { self[$0, c] }
  }
